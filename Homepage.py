...
st.markdown(
    "This homepage allows you to select a year and get some overview statistics of that season."
)
st.markdown(
    "The race pace comparisons page will show you violin plot comparisons of selectable drivers race pace"
    "for a selected race."
)
st.markdown(
    "The hypothetical drivers exchange page runs a machine learning model simulating the race pace of a selected"
    "driver if he replaced another selected driver (same car)."
)
st.markdown(
    "Data covers F1 seasons from 2018 to 2022 (included), and has been mostly retrieved through the "
...
with left_column:
    ...
    drivers_standings_df = pd.read_csv(
        f"./tmp/working_data/drivers_standings/{year}.csv",
        sep=";",
        encoding="UTF-8",
        header=0,
    )
