...
binary_years_path = os.path.join(os.getcwd(), "tmp", "working_data", "binaries_years.csv")

df = pd.read_csv(df_path, sep=";", header=0, encoding="UTF-8")
binary_df = pd.read_csv(binary_path, sep=";", header=0, encoding="UTF-8").drop(
    "Unnamed: 0", axis=1
)
binary_years_df = pd.read_csv(binary_years_path, sep=";", header=0, encoding="UTF-8").drop(
    "Unnamed: 0", axis=1
)
...
def get_hypothetical_predictions(
    ...
    filtered_data = filtered_data.drop(columns=[col for col in filtered_data.columns if "Year_" in col])
